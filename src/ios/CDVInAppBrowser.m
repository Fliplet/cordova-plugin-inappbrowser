...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    //    _localFile = [NSString stringWithString:[url absoluteString]];
...
- (void)createViews
{
    ...
    self.backButton = [[UIBarButtonItem alloc] initWithImage:backArrow style:UIBarButtonItemStylePlain target:self action:@selector(goBack:)];    self.backButton.enabled = YES;
    ...
    self.openInButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(openInAction:)];
    [self.openInButton setEnabled: YES];
    
    NSArray *toolBarItems = @[self.closeButton, flexibleSpaceButton, self.backButton, self.forwardButton, self.openInButton];
    if(!_browserOptions.share) {
        toolBarItems = @[self.closeButton, flexibleSpaceButton, self.backButton, self.forwardButton];
    }
...
/* Allow user to open link through another method */
#pragma mark - Open In Option
- (void)openInAction:(id)sender
{
    if(self.openInDialogue == nil) {
        NSString *activeUrlString = [self.webView stringByEvaluatingJavaScriptFromString:@"window.location.href"];
        NSString *pageTitle = [self.webView stringByEvaluatingJavaScriptFromString:@"document.title"];
        NSURL *url = [[NSURL alloc] initWithString:activeUrlString];
        NSArray* dataToShare  = @[url];
        
        // for the gmail subject we need to attach the title as the first line of the body in order for gmail to catch it.
        if (![activeUrlString hasPrefix:@"file://"]) {
            dataToShare  = @[pageTitle, url];
        }
        
        UIActivityViewController* activityViewController =[[UIActivityViewController alloc] initWithActivityItems:dataToShare applicationActivities:nil];
        
        if (![activeUrlString hasPrefix:@"file://"]) {
            [activityViewController setValue:pageTitle forKey:@"subject"];
        }
        ...
        [[self openInDialogue] dismissMenuAnimated:NO];
        [self setOpenInDialogue:nil];
    }
}
...
//
//    NSURL *appSupportDir = [self applicationDirectory];
//    NSString *filePath= [appSupportDir path];
//
//    _localFile = [filePath stringByAppendingPathComponent:previewDocumentFileName];
//    [fileRemote writeToFile:_localFile atomically:YES];
//}
...
//        NSError *error;
//        BOOL fileExists = [fileManager fileExistsAtPath:_localFile];
//
//        if (fileExists) {
//            BOOL success = [fileManager removeItemAtPath:_localFile error:&error];
